...
def fib_recursive(n):
    if n > 1000:
        raise ValueError("Recursive method is too slow for n > 1000")
    if n <= 1:
        return n
...
for name, (value, time_taken) in results.items():
    if time_taken is None:
        print(f"{name}: Fibonacci({n}) = ERROR (Method failed)")
    else:
        print(f"{name}: Fibonacci({n}) = {str(value)[:50]}..., Time: {time_taken:.6f} seconds")
